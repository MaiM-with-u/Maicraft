...
class ChatHistory:
    ...
    def get_chat_history_str(self) -> str:
        ...
        for chat_event in recent_chats:
            ...
            display_name = chat_event.username
            if chat_event.username == global_config.bot.player_name:
                display_name = "你"

            lines.append(f"{timestamp_str}{display_name}: {chat_event.message}")
        return "\n".join(lines)
    
    def add_chat_history(self, chat_event: ChatEvent):
        self.chat_history.append(chat_event)
        logger.info(f"添加聊天记录: {chat_event.message}")
        if chat_event.username != global_config.bot.player_name:
            self.new_message = True
            if "麦麦" in chat_event.message or global_config.bot.player_name in chat_event.message or global_config.bot.player_name.lower() in chat_event.message:
                # 延迟导入避免循环依赖
                global_thinking_log.add_thinking_log(f"玩家 {chat_event.username} 提到了你，进行回复",type = "notice")
                self.called_message = True
            else:
                global_thinking_log.add_thinking_log(f"玩家 {chat_event.username} 发送了消息：{chat_event.message}",type = "notice")
