...
class BreathEvent(BaseEvent[BreathEventData]):
    ...
    def __init__(self, type: str, gameTick: int, timestamp: float, data: BreathEventData = None):
        """初始化呼吸事件"""
        super().__init__(type, gameTick, timestamp, data)

    def get_description(self) -> str:
        """获取事件的描述信息"""
        oxygen_info = f"氧气水平: {self.data.oxygenLevel}" if self.data.oxygenLevel is not None else ""
        ...
        status_text = f"呼吸状态更新 - {', '.join(info_parts)}" if info_parts else "呼吸状态更新"
