...
await bot.move(x,y,z) -> tuple[bool,BlockPosition,float]:
# 移动到指定位置
# x,y,z是位置的坐标，必须输入x,y,z 三个参数
# 返回值为tuple[bool,BlockPosition,float]，bool为是否成功，BlockPosition为最终位置，float为距离
...
await bot.use_chest(x,y,z,action,item,count) -> tuple[bool,List[Item],int,str]
# 对箱子进行存取操作
# action: "store" 或 "withdraw"
# item: 物品名称
# count: 物品数量
# 返回 (ok, items_after[List[Item]], remaining_slots[int], text)，items_after为箱子操作后的物品列表，remaining_slots为剩余空槽位数

await bot.use_furnace(x,y,z,action,item,count,slot) -> tuple[bool,Item|None,Item|None,Item|None,str]
# 对熔炉执行 put/take；item=物品名称，count=物品数量，slot=槽位 input/fuel/output
# 返回 (ok, input_item, output_item, fuel_item, text)，input_item为输入槽物品，output_item为输出槽物品，fuel_item为燃料槽物品
...
对结果的解释（如果需要的话）：你的计划中是否有遗漏的步骤？为什么代码没有完成任务？聊天记录和执行错误说明了什么问题？
Plan：请一步步说明如何完成该任务。请特别关注背包（Inventory），因为它显示了你拥有的物品。任务是否完成也取决于你最终的背包内容。
Code：
...
4）“Code from the last round” 部分的函数不会被保存或执行，不要复用那里列出的函数。
5）所有变量都应在函数内部定义，函数外定义的内容会被忽略。
6）调用 'bot.chat' 展示中间进度。
7）不要写无限循环或递归函数。
8）不要使用 'bot.on' 或 'bot.once' 注册事件监听器，你绝对不需要它们。
9) 如需要等待，不要使用bot.sleep，使用asyncio.sleep
9）为你的函数起一个有意义的名字（能体现任务内容）。
10）使用正确的 Python 语法并遵循 PEP 8 代码风格。
11）如有需要，在代码开头导入必要的模块。
12) 不要使用任何涉及系统的危险操作！！
