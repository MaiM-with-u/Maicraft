...
_sigint_registered: bool = False


def _register_sigint_cleanup():
    """注册Ctrl+C清理：停止全局3D渲染器并重新抛出KeyboardInterrupt。"""
...
def _register_atexit_cleanup():
    """注册进程退出时的兜底清理。"""
    try:
        atexit.register(lambda: stop_3d_renderer())
    except Exception:
        pass
...
class Renderer3D:
    ...
    def __init__(self, cache: Optional[BlockCache] = None, window_size: Tuple[int, int] = (1200, 800)):
        ...
        # 鼠标锁定相关
        self.mouse_locked = False  # 固定为不锁定，避免鼠标接管相机
    ...
    def _run_render_loop(self):
        ...
        try:
            ...
            # 根据是否锁定为Bot控制设置鼠标行为
            if self.lock_to_bot_camera:
                # 固定Bot视角：显示鼠标且不抓取，防止鼠标输入影响视角
                pygame.mouse.set_visible(True)
                pygame.event.set_grab(False)
                logger.info("固定Bot视角模式：禁用鼠标/键盘控制相机")
            else:
                if self.mouse_locked:
                    pygame.mouse.set_visible(False)
                    pygame.event.set_grab(True)
            ...
            while self.running:
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        elif not self.lock_to_bot_camera and (event.key == pygame.K_F3):
                        ...
                        elif not self.lock_to_bot_camera and event.key == pygame.K_F4:  # 切换相机控制模式
                    ...
                    elif event.type == pygame.MOUSEMOTION and (not self.lock_to_bot_camera) and self.mouse_locked:
                        self._handle_mouse_movement(event.rel)
                
                # 处理键盘输入（若未固定为Bot）
                if not self.lock_to_bot_camera:
    ...
    def _setup_opengl(self):
        ...
        glLoadIdentity()
        gluPerspective(self.fov, self.window_size[0] / self.window_size[1], 0.1, 1000.0)
    ...
    def set_fov(self, fov_degrees: float):
        ...
        try:
            ...
            return
        # 夹取范围
        fov_degrees = max(30.0, min(120.0, fov_degrees))
        ...
        try:
            glMatrixMode(GL_PROJECTION)
            glLoadIdentity()
            aspect = self.window_size[0] / self.window_size[1] if self.window_size[1] != 0 else 1.0
            gluPerspective(self.fov, aspect, 0.1, 1000.0)
            glMatrixMode(GL_MODELVIEW)
        except Exception:
            pass

    def _convert_env_angles(self, yaw_value: float, pitch_value: float) -> Tuple[float, float]:
        """将environment提供的yaw/pitch统一转为度。
        - 若数值范围像弧度（|yaw|<=2π 且 |pitch|<=π），则转换为度；否则视为已是度。
        - yaw 归一化到 [-180, 180]，pitch 限制到 [-89, 89]。
        """
        try:
            is_radian = (abs(yaw_value) <= 2 * math.pi + 1e-3) and (abs(pitch_value) <= math.pi + 1e-3)
            if is_radian:
                yaw_deg = math.degrees(yaw_value)
            ...
            # 取反以匹配OpenGL中前向期望（修正前后反向）
            yaw_deg = -yaw_deg
            # 取反以匹配OpenGL视角期望的抬头/低头方向
            pitch_deg = -pitch_deg
            # pitch限制
            ...
            try:
                # 失败时尽量返回原值（夹取pitch）
                return float(yaw_value), max(-89.0, min(89.0, float(pitch_value)))
            except Exception:
    ...
    def _draw_cube_selective_textured(self, visible_faces: Set[str], block_type: str):
        """只绘制指定的可见面（带纹理坐标，支持按面纹理）。"""
        vertices = [
            [-0.5, -0.5, -0.5],
            [0.5, -0.5, -0.5],
            [0.5, 0.5, -0.5],
    ...
    def _get_face_texture_id(self, block_type: str, face_name: str) -> int:
        ...
        回退顺序：具体面 -> side（对四个侧面）-> 通用 block_type 纹理。
        返回纹理ID，失败返回0。
        """
        # 已有缓存键
        norm = self._normalize_block_type_for_texture(block_type)
        ...
        def try_load(name: str) -> int:
            if name in self.texture_ids:
                return self.texture_ids[name]
            filename = f"{name}.png"
            path = os.path.join(self.texture_base_dir, filename)
            if not os.path.isfile(path):
    ...
    def _render_block_label(self, block: CachedBlock, x: float, y: float, z: float):
        ...
        try:
            ...
            # 更小的标签，减少遮挡
            scale = max(0.2, min(0.5, 3.0 / distance))  # 统一缩小整体比例
